...
func Test${gen_upper_camel(api.name)}(t *testing.T) {
    ...
    gin := &define.Cfg.HttpAddr
    client := http.Client{}
    input := `
        ${input_text}
               `

    request, err := http.NewRequest("POST", fmt.Sprintf("http://%v:%v${api.url}${url_param2text(api.url_param.fields)}", gin.Ip, gin.Port), strings.NewReader(input))
    if err != nil {
        log.Fatal(err)
    }
