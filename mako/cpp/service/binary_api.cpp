...
{   
    ...
    _connection_ptr->async_read(buffer.get(), sizeof(CommandHeader),
            std::bind(&${framework.service_api_class_name}::receive_header, this->shared_from_this(), buffer, std::placeholders::_1, std::placeholders::_2));
    % else:
    // 防止释放
...
void ${framework.service_api_class_name}::request(std:::shared_ptr<char[]> data_ptr, size_t size)
% else:
std::vector<char> ${framework.service_api_class_name}::request(std:::vector<char> data, size_t size)
% endif
{   
        % if framework.no_resp:
...
int ${framework.service_api_class_name}::receive_header(std::shared_ptr<char[]> buffer, const TcpConnection::ErrorCode &ec, size_t length)
{
    if (ec) {
...
% if framework.no_resp:
void ${framework.service_api_class_name}::receive_callback(std:::shared_ptr<char[]> data_ptr)
% else:
std::shared_ptr<char[]> ${framework.service_api_class_name}::receive_callback(std:::shared_ptr<char[]> data_ptr)
% endif
{
...
% for api in framework.server_apis:
% if framework.no_resp:
void ${framework.service_api_class_name}::${api.name}_binary(std:::shared_ptr<char[]> data_ptr)
% else:
std::shared_ptr<char[]> ${framework.service_api_class_name}::${api.name}_binary(std:::shared_ptr<char[]> data_ptr)
% endif
{
    try {
