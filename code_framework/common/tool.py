...
def assert_type(code_type, network):
    # print(code_type, module, type_set.code_types)
    if code_type not in type_set.code_types and network not in type_set.code_types[code_type]:
        print("暂时支持的框架类型[%s] 当前[%s][%s]" % (type_set.code_types, code_type, network))
        assert False
